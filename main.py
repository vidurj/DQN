...
params = {
    'visualize': False,
    'network_type': 'nips',
    'ckpt_file': "ckpt/model_nips_60000",
...
class deep_atari:
    def __init__(self, params):
        ...
        self.engine = emulator(rom_name='breakout.bin', vis=self.params['visualize'],
                               windowname=self.params['network_type'] + '_preview')
        ...
        while self.step < (
            ...
            if self.training and self.step % self.params['learning_interval'] == 0 and self.DB.get_size() > self.params[
                ...
                if self.params['copy_freq'] > 0:
                    feed_dict = {self.targetnet.x: bat_n, self.targetnet.keep_prob: 1.0}
                    q_t = self.sess.run(self.targetnet.y, feed_dict=feed_dict)
                else:
                    feed_dict = {self.qnet.x: bat_n, self.qnet.keep_prob: 1.0}
                ...
                feed_dict = {self.qnet.x: bat_s, self.qnet.q_t: q_t, self.qnet.actions: bat_a,
                             self.qnet.terminals: bat_t, self.qnet.rewards: bat_r, self.qnet.keep_prob: 0.5}
            ...
            if self.training:
                self.params['eps'] = max(self.params['eps_min'],
                                         1.0 - float(self.train_cnt * self.params['learning_interval']) / float(
            ...
            if self.training and self.step > 0 and self.step % self.params['disp_freq'] == 0 and self.DB.get_size() > \
                    self.params['train_start']:
                self.write_log_train()

            if self.training and self.step > 0 and self.step % self.params['eval_freq'] == 0 and self.DB.get_size() > \
                    self.params['train_start']:
                ...
                continue
            if self.training and self.step > 0 and self.step % self.params[
                'steps_per_epoch'] == 0 and self.DB.get_size() > self.params['train_start']:
                self.reset_game()
                self.reset_statistics('all')
                # self.training = False
                continue
            ...
            if self.training:
                self.epi_reward_train += self.reward; self.total_Q_train += self.maxQ
            else:
                self.epi_reward_eval += self.reward; self.total_Q_eval += self.maxQ
    ...
    def write_log_train(self):
        ...
        self.num_epi_train, float(self.total_reward_train) / max(1, self.num_epi_train),
        float(self.total_Q_train) / max(1, self.steps_train), self.total_cost_train / max(1, self.train_cnt_for_disp)))
        sys.stdout.write('    Epsilon : %.3f , Elapsed time : %.1f\n\n' % (self.params['eps'], time.time() - self.s))
        sys.stdout.flush()
        self.log_train.write(
            str(self.step) + ',' + str(self.step // self.params['steps_per_epoch']) + ',' + str(self.train_cnt) + ',')
        self.log_train.write(str(float(self.total_reward_train) / max(1, self.num_epi_train)) + ',' + str(
            float(self.total_Q_train) / max(1, self.steps_train)) + ',')
    ...
    def write_log_eval(self):
        ...
        self.num_epi_eval, float(self.total_reward_eval) / max(1, self.num_epi_eval),
        float(self.total_Q_eval) / max(1, self.params['steps_per_eval'])))
        sys.stdout.write('    Epsilon : %.3f , Elapsed time : %.1f\n\n' % (self.params['eps'], time.time() - self.s))
        sys.stdout.flush()
        self.log_eval.write(
            str(self.step) + ',' + str(self.step // self.params['steps_per_epoch']) + ',' + str(self.train_cnt) + ',')
        self.log_eval.write(str(float(self.total_reward_eval) / max(1, self.num_epi_eval)) + ',' + str(
            float(self.total_Q_eval) / max(1, self.params['steps_per_eval'])) + ',')
    ...
    def select_action(self, st):
        ...
        if np.random.rand() > self.params['eps']:
            ...
            feed_dict = {self.qnet.x: np.reshape(st, (1, 84, 84, 4)), self.qnet.keep_prob: keep_prob}
...
if __name__ == "__main__":
    ...
    for i in range(1, len(sys.argv), 2):
        if sys.argv[i] == '-weight':
            params['ckpt_file'] = sys.argv[i + 1]
        elif sys.argv[i] == '-network_type':
            params['network_type'] = sys.argv[i + 1]
        ...
        elif sys.argv[i] == '-db_size':
            params['db_size'] = int(sys.argv[i + 1])
